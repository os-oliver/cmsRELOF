...
<main>
    <div class="text-center px-2 pt-32">
        <h1 class="text-4xl md:text-5xl font-extrabold text-gray-800 mb-4"><?= htmlspecialchars($dynamicText['t_dokumenti_0d4e0b_901c93']['text'] ?? 'Dokumenti za preuzimanje', ENT_QUOTES, 'UTF-8'); ?></h1>
        <p class="text-lg text-gray-600 max-w-3xl mx-auto mb-10"><?= htmlspecialchars($dynamicText['t_dokumenti_1206bb_cf3b77']['text'] ?? 'Preuzmi sva potrebna dokumenta, obrasce i publikacije Kulturnog Nexusa. Slažemo ih po kategorijama radi lakšeg pronalaženja.', ENT_QUOTES, 'UTF-8'); ?></p>
        ...
        <div class="mx-auto max-w-6xl">
                ...
                <div class="space-y-6">
                    <div class="flex gap-4 items-center">
                        ...
                        <select name="sort" class="px-4 py-3 border rounded-xl w-48">
                            <option value="date_desc"><?= htmlspecialchars($dynamicText['t_dokumenti_7db6d9_765203']['text'] ?? 'Najnoviji prvo', ENT_QUOTES, 'UTF-8'); ?></option>
                            <option value="date_asc"><?= htmlspecialchars($dynamicText['t_dokumenti_2aa5ab_81eb59']['text'] ?? 'Najstariji prvo', ENT_QUOTES, 'UTF-8'); ?></option>
                            <option value="title"><?= htmlspecialchars($dynamicText['t_dokumenti_7d6a12_2794c1']['text'] ?? 'Po nazivu', ENT_QUOTES, 'UTF-8'); ?></option>
                        </select>
                        <button type="submit" class="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-3 rounded-xl whitespace-nowrap"><?= htmlspecialchars($dynamicText['t_dokumenti_93eeed_75c180']['text'] ?? 'Primeni', ENT_QUOTES, 'UTF-8'); ?></button>
                    </div>

                    <div class="w-full">
                        <h3 class="text-sm font-semibold text-gray-700 mb-3"><?= htmlspecialchars($dynamicText['t_dokumenti_68bab4_6dd14c']['text'] ?? 'Kategorije', ENT_QUOTES, 'UTF-8'); ?></h3>
            ...
            <div id="download-notification" class="hidden fixed top-6 right-6 bg-green-600 text-white px-5 py-3 rounded-2xl shadow-lg z-50">
                <div class="flex items-center gap-3"><i class="fa-solid fa-check"></i><span id="download-notification-text"><?= htmlspecialchars($dynamicText['t_dokumenti_78c650_8487f2']['text'] ?? 'Dokument se preuzima...', ENT_QUOTES, 'UTF-8'); ?></span></div>
            </div>
            ...
            <?php if (count($documents) > 0): ?>
                        ...
                        <article class="document-card bg-white rounded-2xl shadow card-hover p-6 fade-in" data-category="<?= $categoryName ?>" data-name="<?= $title ?>">
                            ...
                            <div class="card-footer">
                                <a href="/uploads/documents/<?= $filepath ?>" class="w-full inline-flex items-center justify-center py-2 rounded-lg download-btn text-white font-semibold" download data-id="<?= $fileId ?>" onclick="showDownloadNotification(event, '<?= addslashes($title ?: basename($filepath)) ?>')">
                                    <i class="fa-solid fa-download mr-2"></i><?= htmlspecialchars($dynamicText['t_dokumenti_326242_6ea27e']['text'] ?? 'Preuzmi', ENT_QUOTES, 'UTF-8'); ?></a>
                            </div>
                        </article>
                    <?php endforeach; ?>
                </div>
                ...
                <div class="mt-10 mb-16 flex justify-center items-center gap-4">
                    <?php if ($page > 1): ?>
                        <a href="?<?= http_build_query(array_merge($_GET, ['page' => $page - 1])) ?>" class="px-4 py-2 bg-white rounded-xl shadow hover:bg-gray-100"><?= htmlspecialchars($dynamicText['t_dokumenti_1be9ad_fac577']['text'] ?? '« Prethodna', ENT_QUOTES, 'UTF-8'); ?></a>
                    <?php endif; ?>
                    ...
                    <?php if ($page < $totalPages): ?>
                        <a href="?<?= http_build_query(array_merge($_GET, ['page' => $page + 1])) ?>" class="px-4 py-2 bg-white rounded-xl shadow hover:bg-gray-100"><?= htmlspecialchars($dynamicText['t_dokumenti_b27fa3_b05dc7']['text'] ?? 'Sledeća »', ENT_QUOTES, 'UTF-8'); ?></a>
                    <?php endif; ?>
                </div>
            ...
            <?php else: ?>
                <div class="bg-white rounded-2xl shadow p-10 text-center">
                    ...
                    <h3 class="text-2xl font-bold text-gray-800 mb-4"><?= htmlspecialchars($dynamicText['t_dokumenti_62a7d9_848317']['text'] ?? 'Nema pronađenih dokumenata', ENT_QUOTES, 'UTF-8'); ?></h3>
                    <p class="text-gray-600 max-w-md mx-auto mb-6"><?= htmlspecialchars($dynamicText['t_dokumenti_762533_c353b3']['text'] ?? 'Promenite filtere da biste videli druge dokumente ili proverite kasnije.', ENT_QUOTES, 'UTF-8'); ?></p>
                    <a href="?" class="px-6 py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-bold rounded-xl"><?= htmlspecialchars($dynamicText['t_dokumenti_7122ad_1cc7f0']['text'] ?? 'Resetuj filtere', ENT_QUOTES, 'UTF-8'); ?></a>
                </div>
            <?php endif; ?>
