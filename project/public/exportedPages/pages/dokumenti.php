...
<main>
    <div class="text-center px-2 pt-32">
        <h1 class="text-4xl md:text-5xl font-extrabold text-gray-800 mb-4"><?= htmlspecialchars($dynamicText['t_dokumenti_566c95_901c93']['text'] ?? 'Dokumenti za preuzimanje', ENT_QUOTES, 'UTF-8'); ?></h1>
        <p class="text-lg text-gray-600 max-w-3xl mx-auto mb-10"><?= htmlspecialchars($dynamicText['t_dokumenti_f44d03_cf3b77']['text'] ?? 'Preuzmi sva potrebna dokumenta, obrasce i publikacije Kulturnog Nexusa. Slažemo ih po kategorijama radi lakšeg pronalaženja.', ENT_QUOTES, 'UTF-8'); ?></p>
        ...
        <div class="mx-auto max-w-6xl">
            <form id="filter-form" method="GET" action="" class="bg-white rounded-2xl shadow p-6 mb-8 border border-gray-100">
                <div class="flex flex-col lg:flex-row gap-4 items-center">
                    ...
                    <select name="category" class="px-4 py-3 border rounded-xl">
                        <option value=""><?= htmlspecialchars($dynamicText['t_dokumenti_0bfbb9_b8bbcb']['text'] ?? 'Sve kategorije', ENT_QUOTES, 'UTF-8'); ?></option>
                    ...
                    <select name="sort" class="px-4 py-3 border rounded-xl">
                        <option value="date_desc"><?= htmlspecialchars($dynamicText['t_dokumenti_7b4636_765203']['text'] ?? 'Najnoviji prvo', ENT_QUOTES, 'UTF-8'); ?></option>
                        <option value="date_asc"><?= htmlspecialchars($dynamicText['t_dokumenti_7eca1e_81eb59']['text'] ?? 'Najstariji prvo', ENT_QUOTES, 'UTF-8'); ?></option>
                        <option value="title"><?= htmlspecialchars($dynamicText['t_dokumenti_387485_2794c1']['text'] ?? 'Po nazivu', ENT_QUOTES, 'UTF-8'); ?></option>
                    </select>

                    <button type="submit" class="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-3 rounded-xl"><?= htmlspecialchars($dynamicText['t_dokumenti_4eea54_75c180']['text'] ?? 'Primeni', ENT_QUOTES, 'UTF-8'); ?></button>
                </div>
            </form>
            ...
            <div id="download-notification" class="hidden fixed top-6 right-6 bg-green-600 text-white px-5 py-3 rounded-2xl shadow-lg z-50">
                <div class="flex items-center gap-3"><i class="fa-solid fa-check"></i><span id="download-notification-text"><?= htmlspecialchars($dynamicText['t_dokumenti_bc0c05_8487f2']['text'] ?? 'Dokument se preuzima...', ENT_QUOTES, 'UTF-8'); ?></span></div>
            </div>
            ...
            <?php if (count($documents) > 0): ?>
                        ...
                        <article class="document-card bg-white rounded-2xl shadow card-hover p-6 fade-in" data-category="<?= $categoryName ?>" data-name="<?= $title ?>">
                            ...
                            <div class="card-footer">
                                <a href="/uploads/documents/<?= $filepath ?>" class="w-full inline-flex items-center justify-center py-2 rounded-lg download-btn text-white font-semibold" download data-id="<?= $fileId ?>" onclick="showDownloadNotification(event, '<?= addslashes($title ?: basename($filepath)) ?>')">
                                    <i class="fa-solid fa-download mr-2"></i><?= htmlspecialchars($dynamicText['t_dokumenti_7bebb3_6ea27e']['text'] ?? 'Preuzmi', ENT_QUOTES, 'UTF-8'); ?></a>
                            </div>
                        </article>
                    <?php endforeach; ?>
                </div>
                ...
                <div class="mt-10 mb-16 flex justify-center items-center gap-4">
                    <?php if ($page > 1): ?>
                        <a href="?<?= http_build_query(array_merge($_GET, ['page' => $page - 1])) ?>" class="px-4 py-2 bg-white rounded-xl shadow hover:bg-gray-100"><?= htmlspecialchars($dynamicText['t_dokumenti_424a94_fac577']['text'] ?? '« Prethodna', ENT_QUOTES, 'UTF-8'); ?></a>
                    <?php endif; ?>
                    ...
                    <?php if ($page < $totalPages): ?>
                        <a href="?<?= http_build_query(array_merge($_GET, ['page' => $page + 1])) ?>" class="px-4 py-2 bg-white rounded-xl shadow hover:bg-gray-100"><?= htmlspecialchars($dynamicText['t_dokumenti_4baf09_b05dc7']['text'] ?? 'Sledeća »', ENT_QUOTES, 'UTF-8'); ?></a>
                    <?php endif; ?>
                </div>
            ...
            <?php else: ?>
                <div class="bg-white rounded-2xl shadow p-10 text-center">
                    ...
                    <h3 class="text-2xl font-bold text-gray-800 mb-4"><?= htmlspecialchars($dynamicText['t_dokumenti_171a2e_848317']['text'] ?? 'Nema pronađenih dokumenata', ENT_QUOTES, 'UTF-8'); ?></h3>
                    <p class="text-gray-600 max-w-md mx-auto mb-6"><?= htmlspecialchars($dynamicText['t_dokumenti_0b0a62_c353b3']['text'] ?? 'Promenite filtere da biste videli druge dokumente ili proverite kasnije.', ENT_QUOTES, 'UTF-8'); ?></p>
                    <a href="?" class="px-6 py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-bold rounded-xl"><?= htmlspecialchars($dynamicText['t_dokumenti_6ba0d7_1cc7f0']['text'] ?? 'Resetuj filtere', ENT_QUOTES, 'UTF-8'); ?></a>
                </div>
            <?php endif; ?>
        </div>
    </div>
