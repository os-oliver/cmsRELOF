...
function renderPerPageDropdown(int $currentItemsPerPage): string
{
    ...
    $html .= '<label class="pr-2 text-primary_text" for="per_page">' . __("items_per_page") . '</label>';

    $html .= '<select name="per_page" id="per_page" class="w-20 rounded-xl px-2 py-2 focus:outline-none focus:ring-2 transition-all shadow-sm text-black bg-white/80 backdrop-blur-sm cursor-pointer" onchange="document.getElementById(\'perPageForm\').submit();">';

    foreach ($perPageOptions as $option) {
        $selected = ($currentItemsPerPage === $option) ? 'selected' : '';
        $html .= "<option class=\"text-primary_text\" value=\"{$option}\" {$selected}>{$option}</option>";
    }
