...
class DestinationsPageBuilder extends BasePageBuilder
{
    ...
    private function initializeTexts(): void
    {
        ...
        $latinTexts = [
            ...
            'best_season' => 'Najbolja sezona',
            'recommended_duration' => 'Preporučeno trajanje',
            'budget_per_day' => 'Budžet po danu',
            'city' => 'Grad',
            'highlights' => 'Istaknute atrakcije',
...
{
    $nazivDestinacije = htmlspecialchars($item['fields']['nazivDestinacije'][$locale] ?? '', ENT_QUOTES, 'UTF-8');
    $opis = htmlspecialchars(mb_substr($item['fields']['opis'][$locale] ?? '', 0, $descMaxLength), ENT_QUOTES, 'UTF-8');
    $grad = htmlspecialchars($item['fields']['grad'][$locale] ?? '', ENT_QUOTES, 'UTF-8');
    $najboljaSezona = htmlspecialchars($item['fields']['najboljaSezona'][$locale] ?? '', ENT_QUOTES, 'UTF-8');
    $preporucenoTrajanje = htmlspecialchars($item['fields']['preporucenoTrajanje'][$locale] ?? '', ENT_QUOTES, 'UTF-8');
    $budzetPoDanu = htmlspecialchars($item['fields']['budzetPoDanu'][$locale] ?? '', ENT_QUOTES, 'UTF-8');
    $istaknuteAtrakcije = htmlspecialchars($item['fields']['istaknuteAtrakcije'][$locale] ?? '', ENT_QUOTES, 'UTF-8');
    ...
    $imageSection = $imageUrl
        ...
        : "<div class='absolute inset-0 flex items-center justify-center'>
                ...
                <i class='fas fa-map-marked-alt text-6xl text-indigo-300'></i>
           </div>";
    ...
    $cityRow = $grad
        ? "<div class='flex items-start gap-3'>
               ...
               <div class='flex-1 min-w-0'>
                   <div class='text-xs font-semibold text-gray-500 uppercase tracking-wide mb-0.5'>{$texts['city']}</div>
                   <div class='text-sm font-semibold text-gray-900 truncate'>{$grad}</div>
               </div>
           </div>"
        : '';

    $seasonRow = $najboljaSezona
        ? "<div class='flex items-start gap-3'>
               <div class='flex-shrink-0 w-10 h-10 bg-green-100 rounded-lg flex items-center justify-center'>
                   <i class='fas fa-calendar-alt text-green-600'></i>
               </div>
               <div class='flex-1'>
                   <div class='text-xs font-semibold text-gray-500 uppercase tracking-wide mb-0.5'>{$texts['best_season']}</div>
                   <div class='text-sm font-semibold text-gray-900'>{$najboljaSezona}</div>
               </div>
           </div>"
        : '';

    $durationRow = $preporucenoTrajanje
        ? "<div class='flex items-start gap-3'>
               <div class='flex-shrink-0 w-10 h-10 bg-purple-100 rounded-lg flex items-center justify-center'>
                   <i class='fas fa-clock text-purple-600'></i>
               </div>
               <div class='flex-1'>
                   <div class='text-xs font-semibold text-gray-500 uppercase tracking-wide mb-0.5'>{$texts['recommended_duration']}</div>
                   <div class='text-sm font-semibold text-gray-900'>{$preporucenoTrajanje}</div>
               </div>
           </div>"
        : '';

    $budgetRow = $budzetPoDanu
        ? "<div class='flex items-start gap-3'>
               <div class='flex-shrink-0 w-10 h-10 bg-yellow-100 rounded-lg flex items-center justify-center'>
                   <i class='fas fa-dollar-sign text-yellow-600'></i>
               </div>
               <div class='flex-1'>
                   <div class='text-xs font-semibold text-gray-500 uppercase tracking-wide mb-0.5'>{$texts['budget_per_day']}</div>
                   <div class='text-sm font-semibold text-gray-900'>{$budzetPoDanu}</div>
               </div>
           </div>"
        : '';

    $highlightsSection = $istaknuteAtrakcije
        ? "<div class='mb-4 p-3 bg-blue-50 rounded-lg border border-blue-100'>
               <p class='text-xs font-semibold text-blue-800 mb-1'>{$texts['highlights']}</p>
               <p class='text-sm text-blue-700'>{$istaknuteAtrakcije}</p>
           </div>"
        : '';
    ...
    $replacements = [
        ...
        '{{istaknuteAtrakcije}}' => $istaknuteAtrakcije,
        '{{itemId}}' => $itemId,
        '{{category}}' => $kategorija,
        '{{destinationDetails}}' => $texts['destination_details'] ?? 'Details'
    ];
...
<main class="bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
    <section class="container mx-auto px-4 py-12">
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-gray-900 mb-2">Destinacije</h1>
            <p class="text-gray-600">Istražite naše preporučene destinacije za nezaboravan odmor</p>
        </div>
