...
class IzlozbePageBuilder extends BasePageBuilder
{
...
PHP;
    ...
    $cardTemplate = <<<'PHP'
        <div class="glass-card rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden group transform hover:-translate-y-1 bg-white/30 backdrop-blur-md border border-white/20">
            <div class="relative w-full h-56 overflow-hidden bg-gradient-to-br from-white/50 to-white/30">
                {{imageSection}}
            ...
            <div class="p-5">
                ...
                <div class="mb-4">
                    <a href="/sadrzaj?id={{itemId}}&tip=izlozbe" class="inline-flex items-center gap-2 text-sm font-semibold text-primary">
...
function cardRender(array $item, array $fieldLabels, string $locale, array $texts = [], int $descMaxLength = 120, string $cardTemplate = ''): string
{
    ...
    $defaults = [
        ...
        'view' => 'Pogledaj',
        'no_image_alt' => 'Bez slike izložbe'
    ];
    $texts = array_merge($defaults, $texts);

    // Helper: get nested field safely
    $getField = function($name) use ($item, $locale) {
        if (!empty($item['fields'][$name])) {
            $field = $item['fields'][$name];
            // If field supports locales
            if (is_array($field) && isset($field[$locale])) {
                return $field[$locale];
            }
            return $field;
        }
        // fallback to top-level keys (compatibility)
    ...
    if (is_array($rawNaziv)) {
        // Try common shapes
        if (isset($rawNaziv[0])) $rawNaziv = (string)$rawNaziv[0];
        elseif (isset($rawNaziv['value'])) $rawNaziv = (string)$rawNaziv['value'];
        else $rawNaziv = json_encode($rawNaziv, JSON_UNESCAPED_UNICODE);
    }
    ...
    if ($rawDatum) {
        try {
            ...
            $formattedDatum = $dt->format(LocaleManager::DATE_FORMAT_STRING);
        } catch (Exception $e) {
    ...
    // Build replacements (only required items)
    $replacements = [
    ...
    $tpl = $cardTemplate ?: $GLOBALS['cardTemplate'] ?? '';
