...
function renderTopbar(array $categories, string $searchValue = '', ?int $selectedCategoryId = null, array $texts = []): string
{
    ...
    $html .= "<div class='flex w-full sm:w-auto flex-1 gap-3'>
        <input type='text' name='search' value='{$safeSearchValue}' 
               ...
               class='w-full border border-gray-300 rounded-xl px-5 py-3 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent transition-all shadow-sm bg-white/80 backdrop-blur-sm'>
        <button type='submit' 
                class='bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl transition-all shadow-md hover:shadow-lg font-medium'>
            {$texts['apply_button']}
        </button>
    </div>";
    ...
    $html .= "<div class='flex items-center w-full sm:w-auto'>
        <select name='category' 
                class='w-full sm:w-64 border border-gray-300 rounded-xl px-5 py-3 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent transition-all shadow-sm bg-white/80 backdrop-blur-sm appearance-none cursor-pointer'>
...
function renderPagination(int $currentPage, int $totalPages, int $range = 2): string
{
    ...
    if ($currentPage > 1) {
        ...
        $html .= "<a href='{$prevUrl}' 
                   class='px-4 py-2 bg-white/80 backdrop-blur-sm rounded-xl border border-gray-300 hover:bg-white hover:border-blue-400 transition-all shadow-sm hover:shadow'>
    ...
    if ($start > 1) {
        ...
        $html .= "<a href='{$url}' 
                   class='px-4 py-2 bg-white/80 backdrop-blur-sm rounded-xl border border-gray-300 hover:bg-white hover:border-blue-400 transition-all shadow-sm hover:shadow font-medium'>1</a>";
    ...
    for ($i = $start; $i <= $end; $i++) {
        ...
        $class = $i === $currentPage 
            ? 'px-4 py-2 bg-blue-600 text-white rounded-xl font-semibold shadow-md' 
            : 'px-4 py-2 bg-white/80 backdrop-blur-sm rounded-xl border border-gray-300 hover:bg-white hover:border-blue-400 transition-all shadow-sm hover:shadow font-medium';
    ...
    if ($end < $totalPages) {
        ...
        $html .= "<a href='{$url}' 
                   class='px-4 py-2 bg-white/80 backdrop-blur-sm rounded-xl border border-gray-300 hover:bg-white hover:border-blue-400 transition-all shadow-sm hover:shadow font-medium'>{$totalPages}</a>";
    }
...
<main class="bg-gradient-to-br from-blue-50 to-indigo-50 min-h-screen">
